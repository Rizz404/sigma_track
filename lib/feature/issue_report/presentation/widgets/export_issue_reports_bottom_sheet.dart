...
class ExportIssueReportsBottomSheet extends ConsumerStatefulWidget {
  final ExportIssueReportListUsecaseParams initialParams;

  const ExportIssueReportsBottomSheet({super.key, required this.initialParams});

  @override
  ConsumerState<ExportIssueReportsBottomSheet> createState() =>
      _ExportIssueReportsBottomSheetState();
}

class _ExportIssueReportsBottomSheetState
    extends ConsumerState<ExportIssueReportsBottomSheet> {
  final _formKey = GlobalKey<FormBuilderState>();
  ExportFormat? _selectedFormat;

  @override
  Widget build(BuildContext context) {
    final state = ref.watch(exportIssueReportsProvider);
    final previewData = state.previewData;

    ref.listen(exportIssueReportsProvider, (previous, next) {
              ...
              AppDropdown<String>(
                ...
                onChanged: (value) {
                  if (value != null) {
                    ...
                    ref.read(exportIssueReportsProvider.notifier).reset();
                  }
                },
              ),
              const SizedBox(height: 24),
  ...
  Future<void> _handleExport() async {
    if (_formKey.currentState?.saveAndValidate() ?? false) {
      ...
      await ref
          .read(exportIssueReportsProvider.notifier)
          .exportIssueReports(params);
  ...
  Future<void> _handleOpenAndSave(Uint8List fileData) async {
    try {
      ...
      final fileName = 'issue_reports_export_$timestamp';
