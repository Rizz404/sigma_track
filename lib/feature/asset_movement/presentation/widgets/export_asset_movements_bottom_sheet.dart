...
class ExportAssetMovementsBottomSheet extends ConsumerStatefulWidget {
  final ExportAssetMovementListUsecaseParams initialParams;

  const ExportAssetMovementsBottomSheet({
    super.key,
    required this.initialParams,
  });

  @override
  ConsumerState<ExportAssetMovementsBottomSheet> createState() =>
      _ExportAssetMovementsBottomSheetState();
}

class _ExportAssetMovementsBottomSheetState
    extends ConsumerState<ExportAssetMovementsBottomSheet> {
  final _formKey = GlobalKey<FormBuilderState>();
  ExportFormat? _selectedFormat;

  @override
  Widget build(BuildContext context) {
    final state = ref.watch(exportAssetMovementsProvider);
    final previewData = state.previewData;

    ref.listen(exportAssetMovementsProvider, (previous, next) {
              ...
              AppDropdown<String>(
                ...
                onChanged: (value) {
                  if (value != null) {
                    ...
                    ref.read(exportAssetMovementsProvider.notifier).reset();
                  }
                },
              ),
              const SizedBox(height: 24),
  ...
  Future<void> _handleExport() async {
    if (_formKey.currentState?.saveAndValidate() ?? false) {
      ...
      await ref
          .read(exportAssetMovementsProvider.notifier)
          .exportAssetMovements(params);
  ...
  Future<void> _handleOpenAndSave(Uint8List fileData) async {
    try {
      ...
      final fileName = 'asset_movements_export_$timestamp';
