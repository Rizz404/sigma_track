...
class ExportMaintenanceRecordsBottomSheet extends ConsumerStatefulWidget {
  final ExportMaintenanceRecordListUsecaseParams initialParams;

  const ExportMaintenanceRecordsBottomSheet({
    super.key,
    required this.initialParams,
  });

  @override
  ConsumerState<ExportMaintenanceRecordsBottomSheet> createState() =>
      _ExportMaintenanceRecordsBottomSheetState();
}

class _ExportMaintenanceRecordsBottomSheetState
    extends ConsumerState<ExportMaintenanceRecordsBottomSheet> {
  final _formKey = GlobalKey<FormBuilderState>();
  ExportFormat? _selectedFormat;

  @override
  Widget build(BuildContext context) {
    final state = ref.watch(exportMaintenanceRecordsProvider);
    final previewData = state.previewData;

    ref.listen(exportMaintenanceRecordsProvider, (previous, next) {
              ...
              AppDropdown<String>(
                ...
                onChanged: (value) {
                  if (value != null) {
                    ...
                    ref.read(exportMaintenanceRecordsProvider.notifier).reset();
                  }
                },
              ),
              const SizedBox(height: 24),
  ...
  Future<void> _handleExport() async {
    if (_formKey.currentState?.saveAndValidate() ?? false) {
      ...
      this.logPresentation('Exporting maintenance records: $params');

      await ref
          .read(exportMaintenanceRecordsProvider.notifier)
          .exportMaintenanceRecords(params);
  ...
  Future<void> _handleOpenAndSave(Uint8List fileData) async {
    try {
      ...
      final fileName = 'maintenance_records_export_$timestamp';
