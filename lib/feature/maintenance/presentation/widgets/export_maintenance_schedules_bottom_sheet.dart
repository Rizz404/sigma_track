...
class ExportMaintenanceSchedulesBottomSheet extends ConsumerStatefulWidget {
  final ExportMaintenanceScheduleListUsecaseParams initialParams;

  const ExportMaintenanceSchedulesBottomSheet({
    super.key,
    required this.initialParams,
  });

  @override
  ConsumerState<ExportMaintenanceSchedulesBottomSheet> createState() =>
      _ExportMaintenanceSchedulesBottomSheetState();
}

class _ExportMaintenanceSchedulesBottomSheetState
    extends ConsumerState<ExportMaintenanceSchedulesBottomSheet> {
  final _formKey = GlobalKey<FormBuilderState>();
  ExportFormat? _selectedFormat;

  @override
  Widget build(BuildContext context) {
    final state = ref.watch(exportMaintenanceSchedulesProvider);
    final previewData = state.previewData;

    ref.listen(exportMaintenanceSchedulesProvider, (previous, next) {
              ...
              AppDropdown<String>(
                ...
                onChanged: (value) {
                  if (value != null) {
                    ...
                    ref
                        .read(exportMaintenanceSchedulesProvider.notifier)
                        .reset();
                  }
                },
              ),
  ...
  Future<void> _handleExport() async {
    if (_formKey.currentState?.saveAndValidate() ?? false) {
      ...
      this.logPresentation('Exporting maintenance schedules: $params');

      await ref
          .read(exportMaintenanceSchedulesProvider.notifier)
          .exportMaintenanceSchedules(params);
  ...
  Future<void> _handleOpenAndSave(Uint8List fileData) async {
    try {
      ...
      final fileName = 'maintenance_schedules_export_$timestamp';
