...
class ExportUsersBottomSheet extends ConsumerStatefulWidget {
  final ExportUserListUsecaseParams initialParams;

  const ExportUsersBottomSheet({super.key, required this.initialParams});

  @override
  ConsumerState<ExportUsersBottomSheet> createState() =>
      _ExportUsersBottomSheetState();
}

class _ExportUsersBottomSheetState
    extends ConsumerState<ExportUsersBottomSheet> {
  final _formKey = GlobalKey<FormBuilderState>();
  ExportFormat? _selectedFormat;

  @override
  Widget build(BuildContext context) {
    final state = ref.watch(exportUsersProvider);
    final previewData = state.previewData;

    ref.listen(exportUsersProvider, (previous, next) {
              ...
              AppDropdown<String>(
                ...
                onChanged: (value) {
                  if (value != null) {
                    ...
                    ref.read(exportUsersProvider.notifier).reset();
                  }
                },
              ),
              const SizedBox(height: 24),
  ...
  Future<void> _handleExport() async {
    if (_formKey.currentState?.saveAndValidate() ?? false) {
      ...
      await ref.read(exportUsersProvider.notifier).exportUsers(params);
